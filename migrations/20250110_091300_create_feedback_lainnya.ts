...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable('feedback_lainnya', (table) => {
    ...
    table.integer('prasarana_id').unsigned().nullable(); // spesifik ruangan yang difeedback
    table.integer('rating_fasilitas').notNullable().defaultTo(5); // rating 1-5
    ...
    table.integer('rating_akses').notNullable().defaultTo(5); // aksesibilitas
    table.text('komentar_akses').nullable();
    table.text('masalah_teknis').nullable(); // masalah teknis yang ditemui
    table.text('saran_perbaikan').nullable(); // saran perbaikan spesifik
    table.boolean('akan_gunakan_lagi').defaultTo(true); // apakah akan menggunakan lagi
    table.text('alasan_tidak_gunakan').nullable(); // alasan jika tidak akan gunakan lagi
  ...
  // Create view for facility rating statistics
  await knex.raw(`
  ...
  // Create view for monthly feedback trends
  await knex.raw(`
